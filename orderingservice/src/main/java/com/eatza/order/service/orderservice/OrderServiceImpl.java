...
import com.eatza.order.feignclient.DeliverServiceClient;
...
public class OrderServiceImpl implements OrderService {
	...
	@Autowired
	private KafkaTemplate<String, DeliveryResponseDto> kafkaTemplate;
   ...
   @Autowired
   DeliverServiceClient deliveryServiceClient;
	...
	public Order placeOrder(OrderRequestDto orderRequest) throws OrderException {
		...
		for(OrderedItemsDto itemDto: itemsDtoList) {
			...
			try {
				...
				DeliveryResponseDto deliveringOrder=new DeliveryResponseDto(savedOrder.getId(),savedOrder.getCustomerId(),savedOrder.getRestaurantId());
				System.out.println("object" +deliveringOrder);
				//ResponseEntity<DeliveryResponseDto> deliver=this.deliveryServiceClient.loggingOrder(deliveringOrder);
				Map<String, Object> headers = new HashMap<>();
				headers.put(KafkaHeaders.TOPIC, topicName);
				kafkaTemplate.send(new GenericMessage<DeliveryResponseDto>(deliveringOrder, headers));
				System.out.println("Order\n" +savedOrder);
				//System.out.println("Delivery\n" +deliver.getBody());
				savedOrder.setLoggDeliver("Yes");
